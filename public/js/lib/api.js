...
async function request(path, options = {}) {
  ...
  // the browser sets its own multipart boundary. JSON bodies (plain
  // strings) do.
...
const mapSettings = (s) => s ? ({
  ...
  language: s.language, searchVisible: !!s.search_visible, saveContactEnabled: s.save_contact_enabled !== 0
}) : null;
...
const mapProfile = (u) => u ? ({
  ...
  socials: {
    ...
    facebook: u.social_facebook ?? u.socials?.facebook ?? ''
  }
}) : null;
...
class MochimoAPI {
  ...
  async uploadMedia(file) {
    ...
    // boundary itself. request() only adds Content-Type when a plain body
    // is passed, so a FormData body correctly skips that.
    return request('/api/upload', { method: 'POST', body: form });
  }

  async uploadImage(file) {
    const data = await this.uploadMedia(file);
    if (data.type && data.type !== 'image') throw new ApiError('Please upload an image.', 400);
    return data.url;
  }
  ...
  async getPublicAccount(username) {
    try {
      ...
      return { profile: mapProfile(data.profile), links: (data.links || []).map(mapLink), appearance: mapAppearance(data.appearance), settings: { saveContactEnabled: data.settings?.save_contact_enabled !== 0 } };
