...
import { renderPhotographerTemplate } from './photographerTemplate.js';
...
const LINK_TYPE_META = {
  ...
  facebook: { icon: 'facebook', badge: 'pv-badge-facebook' },
};
...
function renderProfilePreview(container, { profile, links, appearance }) {
  if (!container) return;
  const screenEl = container.querySelector('.phone-screen') || container;

  // Photographer theme is a fully separate template — isolated in its own
  // file, chosen here, everything below is unchanged classic-theme code.
  if (appearance.theme === 'photographer') {
    screenEl.style.removeProperty('--profile-bg');
    screenEl.style.removeProperty('--profile-text');
    screenEl.style.removeProperty('--profile-text-muted');
    renderPhotographerTemplate(screenEl, { profile, links, appearance });
    return;
  }
